...
import entrants.pacman.Squillyprice01.edu.southwestern.tasks.mspacman.sensors.blocks.MsPacManSensorBlock;
import entrants.pacman.Squillyprice01.edu.southwestern.tasks.mspacman.sensors.directional.reachfirst.VariableDirectionCloserToTargetThanThreatGhostBlock;
...
public class DirectionalReachSafetyBeforeThreatGhostBlock extends MsPacManSensorBlock {
	...
	public int incorporateSensors(double[] inputs, int in, GameFacade gf, int lastDirection) {
		...
		ArrayList<Set<Integer>> safePoints = gf.junctionsAtDepth(current, depth, escapeToPowerPills,
				escapeNodes.getLastNodeVisited());
			...
			for (int j = 0; j < GameFacade.NUM_DIRS; j++) {
				...
				if (wall) {
					...
					if (depthSafety[dir] == i - 1) {
						...
						safe = VariableDirectionCloserToTargetThanThreatGhostBlock
								.canReachAnyTargetSafelyInDirection(gf, targets, dir);
						if (safe) {
							depthSafety[dir] = i;
						}
					}
