package entrants.pacman.Squillyprice01.edu.southwestern.tasks.mspacman.sensors.blocks.nearestfarthest;
...
public abstract class NearestFarthestIndexBlock extends MsPacManSensorBlock {
	...
	public int incorporateSensors(final double[] inputs, int in, final GameFacade gf, final int currentDir,
		...
		int targetDir = -1;
		if (targets.length > 0) {
			int target = nearest ? gf.getClosestNodeIndexFromNodeIndex(current, targets)
					: gf.getFarthestNodeIndexFromNodeIndex(current, targets);
			targetDir = gf.getNextPacManDirTowardsTarget(target);
			// if (watch) {
			// gf.addPoints(CombinatoricUtilities.colorFromInt(typeOfTarget().length()),
			// gf.getShortestPath(current, target));
			// }
		}
