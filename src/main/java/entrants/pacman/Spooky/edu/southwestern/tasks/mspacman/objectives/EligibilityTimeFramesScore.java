...
import entrants.pacman.Spooky.edu.southwestern.tasks.mspacman.agentcontroller.pacman.MultinetworkSelectorMsPacManController;
import entrants.pacman.Spooky.edu.southwestern.tasks.mspacman.agentcontroller.pacman.NNMsPacMan;
import entrants.pacman.Spooky.edu.southwestern.tasks.mspacman.agentcontroller.pacman.NNPacManController;
	...
	public static double fitness(LinkedList<Interval<Integer>> usageTimeFrames, List<Integer> eatTimes) {
		...
		while (itr.hasNext()) {
			...
			for (int i = currentFrame.t1; i < currentFrame.t2 && i < eligibility.length; i++) {
				...
				// currentEligibility = Math.min(1.0, currentEligibility /
				// lambda); // increasing
		...
		// }
		// Plot.linePlot(new DrawingPanel(600, 600, "Eligibility"), -0.5, 1.5,
		// plotLine, Color.RED);
