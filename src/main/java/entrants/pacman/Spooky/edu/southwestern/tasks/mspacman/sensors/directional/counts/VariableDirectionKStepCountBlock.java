 ...
 */
package entrants.pacman.Spooky.edu.southwestern.tasks.mspacman.sensors.directional.counts;
...
public abstract class VariableDirectionKStepCountBlock extends VariableDirectionBlock {
	 ...
	 *            the game state
	 * @param visited
	 *            locations that have already been visited along the given path.
	 *            Note that the path may loop on to itself, and then continue on
	 *            into new areas.
	 * @param targets
	 *            objects that should be counted if they occur along the path
	 * @param sourceLocation
	 *            where pacman came from. Cannot go back to this location.
	 * @param currentLocation
	 *            where pacman currently is
	 * @param remainingSteps
	 *            How much further to continue along the path.
	 * @return maximum number of targets found along all available paths
	 */
	public static int countMembersAlongPath(GameFacade gf, ArrayList<Integer> visited, int[] targets,
			int sourceLocation, int currentLocation, int remainingSteps, boolean max) {
		...
		if (remainingSteps == 0) {
			...
			for (int i = 0; i < neighbors.length; i++) {
				if (neighbors[i] != -1) {
					int count = countMembersAlongPath(gf, visited, targets, currentLocation, neighbors[i],
							remainingSteps - 1, max);
