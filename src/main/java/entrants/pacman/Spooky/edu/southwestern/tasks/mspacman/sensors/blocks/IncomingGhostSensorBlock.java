package entrants.pacman.Spooky.edu.southwestern.tasks.mspacman.sensors.blocks;
	...
	public int incorporateSensors(double[] inputs, int in, GameFacade gf, int currentDir) {
		...
		for (int i = 0; i < CommonConstants.numActiveGhosts; i++) {
			if (threats ? gf.isGhostThreat(i) : gf.isGhostEdible(i)) {
				...
				assert gf.getGhostCurrentNodeIndex(i) == current
						|| ghostPathToPacMan.length > 0 : "Ghost Path too short: ghost index: " + i + ":ghost loc:"
								+ gf.getGhostCurrentNodeIndex(i) + ":pacman loc:" + current + ":threats:" + threats;
				assert ghostPathToPacMan[ghostPathToPacMan.length
						- 1] == current : "Ghost path does not end in pacman's location: " + current + ":"
								+ Arrays.toString(ghostPathToPacMan);
				// gf.addPoints(CombinatoricUtilities.colorFromInt(i),
				// ghostPathToPacMan);
				int source = ghostPathToPacMan.length > 1 ? ghostPathToPacMan[ghostPathToPacMan.length - 2]
						: gf.getGhostCurrentNodeIndex(i);
				int dir = gf.getNextPacManDirTowardsTarget(source);
				// Compare shortest distance to path distance to see if
				// approaching via shortest route
		...
		for (int j = 0; j < GameFacade.NUM_DIRS; j++) {
			...
			if (wall) {
				...
				inputs[in++] = (!opposite && approachingFromDirection[dir])
						|| (opposite && !approachingFromDirection[dir]) ? 1 : 0;
			}
		}
		return in;
	}
