package entrants.pacman.Spooky.edu.southwestern.tasks.mspacman.facades;
...
import entrants.pacman.Spooky.edu.southwestern.tasks.mspacman.ghosts.GhostComparator;
...
import entrants.pacman.Spooky.edu.southwestern.util.datastructures.Quad;
import entrants.pacman.Spooky.edu.southwestern.util.datastructures.Triple;
...
public class GameFacade {
	...
	public entrants.pacman.Spooky.oldpacman.game.Game oldG = null;
	 ...
	 */
	public static entrants.pacman.Spooky.oldpacman.game.Constants.MOVE indexToMove(int index) {
		switch (index) {
		case 0:
			return entrants.pacman.Spooky.oldpacman.game.Constants.MOVE.UP;
		case 1:
			return entrants.pacman.Spooky.oldpacman.game.Constants.MOVE.RIGHT;
		case 2:
			return entrants.pacman.Spooky.oldpacman.game.Constants.MOVE.DOWN;
		case 3:
			return entrants.pacman.Spooky.oldpacman.game.Constants.MOVE.LEFT;
		default:
			return null;
		}
	}
	 ...
	 */
	public static int moveToIndex(entrants.pacman.Spooky.oldpacman.game.Constants.MOVE m) {
		switch (m) {
		case UP:
			return 0;
		case RIGHT:
	 ...
	 */
	public static entrants.pacman.Spooky.oldpacman.game.Constants.GHOST indexToGhost(int index) {
		assert index >= 0 && index <= 3 : "Must be a valid ghost index: " + index;
		switch (index) {
		case 0:
			return entrants.pacman.Spooky.oldpacman.game.Constants.GHOST.BLINKY;
		case 1:
			return entrants.pacman.Spooky.oldpacman.game.Constants.GHOST.PINKY;
		case 2:
			return entrants.pacman.Spooky.oldpacman.game.Constants.GHOST.INKY;
		case 3:
			return entrants.pacman.Spooky.oldpacman.game.Constants.GHOST.SUE;
		default:
	 ...
	 */
	public static int ghostToIndex(entrants.pacman.Spooky.oldpacman.game.Constants.GHOST ghost) {
		switch (ghost) {
		case BLINKY:
			return 0;
		case PINKY:
	 ...
	 */
	public static int getNeighbourInDirection(entrants.pacman.Spooky.oldpacman.game.Game g, int current, entrants.pacman.Spooky.oldpacman.game.Constants.MOVE move) {
	 ...
	 * @return neighboring node index
	 */
	public static int getNeighbourInDirection(pacman.game.Game g, int current, entrants.pacman.Spooky.oldpacman.game.Constants.MOVE move) {
		return neighbors(g, current)[moveToIndex(move)];
	}
	 ...
	 */
	public static int[] neighbors(entrants.pacman.Spooky.oldpacman.game.Game gs, int currentNodeIndex) {
		assert currentNodeIndex != -1 : "-1 is not a valid node index";
		entrants.pacman.Spooky.oldpacman.game.Constants.MOVE[] possible = gs.getPossibleMoves(currentNodeIndex);
		...
		for (int i = 0; i < possible.length; i++) {
			...
			assert!((possible[i].equals(entrants.pacman.Spooky.oldpacman.game.Constants.MOVE.UP) && (cx != nx || ny >= cy))
					|| (possible[i].equals(entrants.pacman.Spooky.oldpacman.game.Constants.MOVE.DOWN) && (cx != nx || ny <= cy))
					|| (possible[i].equals(entrants.pacman.Spooky.oldpacman.game.Constants.MOVE.LEFT) && (cy != ny || (nx >= cx && nx != 108 & cx != 0)))
					|| (possible[i].equals(entrants.pacman.Spooky.oldpacman.game.Constants.MOVE.RIGHT) && (cy != ny
	 ...
	 * @param g game
	 */
	public GameFacade(entrants.pacman.Spooky.oldpacman.game.Game g) {
		oldG = g;
	}
	...
	public int[] getShortestPath(int from, int to) {
		...
		//flow control to differentiate between entrants.pacman.Spooky.oldpacman and popacman
		if(oldG != null) {
	...
	public boolean isGhostEdible(int ghostIndex) {
		//flow control between popacman and entrants.pacman.Spooky.oldpacman
		if(oldG == null) {
	...
	public int getClosestNodeIndexFromNodeIndex(int current, int[] targets) {
		...
		return oldG == null ?
				...
				oldG.getClosestNodeIndexFromNodeIndex(current, targets, entrants.pacman.Spooky.oldpacman.game.Constants.DM.PATH);
	}
	...
	public void addPoints(Color c, int[] nodes) {
		if(oldG == null) {
			...
			if (nodes.length > 0) {
				entrants.pacman.Spooky.oldpacman.game.GameView.addPoints(oldG, c, ArrayUtil.filter(nodes, -1));
			}
		}
	}
	...
	public double getPathDistance(int from, int to) {
		...
		return oldG == null ?
				...
				oldG.getDistance(from, to, entrants.pacman.Spooky.oldpacman.game.Constants.DM.PATH);
	}
	...
	public boolean pacmanHittingWall() {
		return oldG == null ?
				...
				oldG.getPacmanLastMoveMade().equals(entrants.pacman.Spooky.oldpacman.game.Constants.MOVE.NEUTRAL);
	}
	...
	public void addLines(Color c, int from, int to) {
		...
		if(oldG == null) {
			...
			entrants.pacman.Spooky.oldpacman.game.GameView.addLines(oldG, c, from, to);
		}
	...
	public int getNextGhostDirTowards(int whichGhost, int to) {
		...
		return oldG == null ?
				...
				moveToIndex(oldG.getApproximateNextMoveTowardsTarget(getGhostCurrentNodeIndex(whichGhost), to,
						oldG.getGhostLastMoveMade(indexToGhost(whichGhost)), entrants.pacman.Spooky.oldpacman.game.Constants.DM.PATH));
	}	
	...
	public int getRestrictedNextDir(int from, int to, int lastDir) {
		...
		return oldG == null ?
				...
				moveToIndex(oldG.getApproximateNextMoveTowardsTarget(from, to, indexToMove(lastDir), entrants.pacman.Spooky.oldpacman.game.Constants.DM.PATH));
	}
	...
	public int getNextPacManDirTowardsTarget(int to) {
		...
		return oldG == null ?
				...
				moveToIndex(oldG.getNextMoveTowardsTarget(oldG.getPacmanCurrentNodeIndex(), to, entrants.pacman.Spooky.oldpacman.game.Constants.DM.PATH));
	}
	...
	public int getNextPacManDirAwayFromTarget(int to) {
		...
		return oldG == null ?
				...
				moveToIndex(oldG.getNextMoveAwayFromTarget(oldG.getPacmanCurrentNodeIndex(), to, entrants.pacman.Spooky.oldpacman.game.Constants.DM.PATH));
	}
	...
	public double getGhostPathDistance(int ghostIndex, int toNodeIndex) {
		...
		return oldG == null ?
			...
			oldG.getDistance(getGhostCurrentNodeIndex(ghostIndex), toNodeIndex,
					oldG.getGhostLastMoveMade(indexToGhost(ghostIndex)), entrants.pacman.Spooky.oldpacman.game.Constants.DM.PATH);
	}
	...
	public int getGhostTravelTime(int ghostIndex, int toNodeIndex) {
		...
		if(oldG == null) {
			...
			effectiveEdibleTime = Math.min(edibleTime, distance * entrants.pacman.Spooky.oldpacman.game.Constants.GHOST_SPEED_REDUCTION);
			return effectiveEdibleTime + distance
					- ((int) Math.ceil(effectiveEdibleTime / entrants.pacman.Spooky.oldpacman.game.Constants.GHOST_SPEED_REDUCTION));
		}
	...
	public int getNumberOfEdibleGhosts() {
		...
		for (int i = 0; i < CommonConstants.numActiveGhosts; i++) {
			...
			//flow control to differentiate between popacman and entrants.pacman.Spooky.oldpacman
			if(oldG == null) {
	...
	public int getFarthestNodeIndexFromNodeIndex(int current, int[] targets) {
		...
		return oldG == null ?
				...
				oldG.getFarthestNodeIndexFromNodeIndex(current, targets, entrants.pacman.Spooky.oldpacman.game.Constants.DM.PATH);
	}	
	...
	public int getNextMoveTowardsTarget(int from, int to) {
		...
		return oldG == null ?
				...
				moveToIndex(oldG.getNextMoveTowardsTarget(from, to, entrants.pacman.Spooky.oldpacman.game.Constants.DM.PATH));
	}
	...
	public void advanceGame(int pacManDir, int[] ghostDirs) {
		...
		if(oldG == null) {
			...
			EnumMap<entrants.pacman.Spooky.oldpacman.game.Constants.GHOST, entrants.pacman.Spooky.oldpacman.game.Constants.MOVE> myMoves = 
					new EnumMap<entrants.pacman.Spooky.oldpacman.game.Constants.GHOST, entrants.pacman.Spooky.oldpacman.game.Constants.MOVE>(entrants.pacman.Spooky.oldpacman.game.Constants.GHOST.class);
	 ...
	 */
	private static boolean anyRequiresAction(entrants.pacman.Spooky.oldpacman.game.Game newG) {
		for (int i = 0; i < CommonConstants.numActiveGhosts; i++) {
	 ...
	 */
	private static int maxEdibleTime(entrants.pacman.Spooky.oldpacman.game.Game newG) {
		int max = -1;
		for (int i = 0; i < CommonConstants.numActiveGhosts; i++) {
	 ...
	 */
	private boolean anyIsEdible(entrants.pacman.Spooky.oldpacman.game.Game newG) {
		for (int i = 0; i < CommonConstants.numActiveGhosts; i++) {
	...
	public int getNumberOfLairGhosts() {
		int count = 0;
		//branching to differentiate between popacman and entrants.pacman.Spooky.oldpacman
		if(oldG == null) {
			...
			for (int i = 0; i < entrants.pacman.Spooky.oldpacman.game.Constants.NUM_GHOSTS; i++) {
				if (this.getGhostLairTime(i) > 0) {
					count++;
				}
			}
