...
public class NNMultitaskSelectorCheckEachDirectionPacManController extends NNDirectionalPacManController {
	...
	public NNMultitaskSelectorCheckEachDirectionPacManController(Genotype<? extends Network> policy,
			Genotype<? extends Network> preference, VariableDirectionBlock safe) {
		...
		for (int i = 0; i < GameFacade.NUM_DIRS; i++) {
			directionalPolicyNetworks[i] = policy.getPhenotype(); // Each is a
																	// different
																	// copy
			directionalPreferenceNetworks[i] = preference.getPhenotype(); // Each
																			// is
																			// a
																			// different
																			// copy
		...
		for (int i = 0; i < neighbors.length; i++) {
			if (neighbors[i] != -1) {
				...
				if (CommonConstants.checkEachFlushWalls) {
					this.directionalPolicyNetworks[i].flush(); // Nothing sensed
																// from wall
				}
			}
		}
