...
import entrants.pacman.Vilhelm_Von_Tweann.edu.southwestern.util.datastructures.Interval;
...
public class MultinetworkSelectorMsPacManController<T extends Network> extends MultinetworkMsPacManController<T> {
	...
	public MultinetworkSelectorMsPacManController(Genotype<T>[] genotypes,
	...
	protected void drawModeUsage(GameFacade game, Graphics2D g, int[] actions, int action, int currentScale, int mode) {
		...
		if (lastMode != -1) {
			g.drawLine(scaledTime(game.getCurrentLevelTime() - 1), scaledMode(lastMode),
					scaledTime(game.getCurrentLevelTime()), scaledMode(mode));
		}
	...
	protected void modeUpdate(GameFacade game, int mode) {
		if (mode != lastMode) {
			...
			// System.out.println(mode + ":" + nn.numInputs() + ":" +
			// inputMediator.getClass().getSimpleName());
			if (CommonConstants.showNetworks && animateNetwork && nn instanceof TWEANN) {
				...
				TWEANN.panel = new DrawingPanel(TWEANN.NETWORK_VIEW_DIM, TWEANN.NETWORK_VIEW_DIM,
						"Multinetwork " + mode);
