 ...
 */
package entrants.pacman.Vilhelm_Von_Tweann.edu.southwestern.tasks.mspacman.ensemble;
...
/**
 * The general idea is to be a specialization of the AverageArbitrator. The
 * specialization comes in when certain modes are entirely disabled, so as to
 * removed the choice or influence of a certain mode.
 * 
 * @author Jacob Schrum
 */
public abstract class OverlappingArbitrator extends AverageArbitrator {
	...
	public double[] newDirectionalPreferences(GameFacade game, double[][] preferences) {
		assert modes() == preferences.length : "Modes " + modes() + " not equal to preference groups "
				+ preferences.length;
		...
		for (int i = 0; i < consider.length; i++) {
			if (consider[i]) {
				// System.out.println("\tKeeping preferences: " +
				// Arrays.toString(preferences[i]));
