package entrants.pacman.Vilhelm_Von_Tweann.edu.southwestern.tasks.mspacman.sensors.blocks.distancedifference;
...
public class EscapeNodeDistanceDifferenceBlock extends PacManVsThreatDistanceDifferencesBlock {
	...
	public EscapeNodeDistanceDifferenceBlock(NodeCollection en, boolean ghostDistances, boolean pacmanDistances) {
		this(en, ghostDistances, pacmanDistances, Parameters.parameters.integerParameter("escapeNodeDepth"),
				!Parameters.parameters.booleanParameter("ignorePillScore"),
				!Parameters.parameters.booleanParameter("noPowerPills"),
				!Parameters.parameters.booleanParameter("noPowerPills"));
	}

	public EscapeNodeDistanceDifferenceBlock(NodeCollection en, boolean ghostDistances, boolean pacmanDistances,
			int simulationDepth, boolean futurePillsEaten, boolean futureGhostsEaten, boolean futurePowerPillsEaten) {
		super(ghostDistances, pacmanDistances, simulationDepth, futurePillsEaten, futureGhostsEaten,
				futurePowerPillsEaten);
		escapeNodes = en;
	}
	...
	public int[] getTargets(GameFacade gf) {
		...
		assert gf.allNodesInMaze(result) : "Some members of result are not in maze. Current: " + gf.getCurrentLevel()
				+ ":targets:" + Arrays.toString(result) + ":junctions:" + Arrays.toString(gf.getJunctionIndices())
				+ ":power pills:" + Arrays.toString(gf.getActivePowerPillsIndices());
		return result;
	}
	 ...
	 *
	 * @param actualJunctions
	 *            Taken directly from the GameFacade
	 * @param processedJunctions
	 *            Returned by the NodeCollection
	 * @return
	 */
	// public static boolean tempJunctionArrayComparison(int[] actualJunctions,
	// int[] processedJunctions) {
	// int overlap = 0;
	// for (int i = 0; i < actualJunctions.length; i++) {
	// int occurrences = ArrayUtil.countOccurrences(actualJunctions[i],
	// processedJunctions);
	...
	// System.out.println("actualJunctions: " +
	// Arrays.toString(actualJunctions));
	// return false;
	// } else if (occurrences == 1 || occurrences == 2) { // Two allowed because
	// of adding back last visited
	// overlap++;
	// }
	// }
	...
	public void addDifferencesInGhostPacManDistancesToLocation(double[] inputs, int in, int[] neighbors,
		...
		assert previous.allNodesInMaze(targets) : "Some members of targets are not in maze "
				+ previous.getCurrentLevel();
		...
		if (callDepth > 0) {
			...
			if (previous.getCurrentLevel() != gs.getCurrentLevel()) {
				...
				assert previous.allNodesInMaze(targets) : "Nodes update made some targets not in maze "
						+ previous.getCurrentLevel();
			}
		}
		...
		// System.out.println("mazeBefore:"+mazeBefore+":gs.getMazeIndex():" +
		// gs.getMazeIndex()+":previous.getMazeIndex():" +
		// previous.getMazeIndex());
		// System.out.println("callDepth:" + callDepth);
		// System.out.println("originalTargets:" +
		// Arrays.toString(originalTargets));
		// System.out.println("targets:" + Arrays.toString(targets));
		// System.exit(1);
		// }
		super.addDifferencesInGhostPacManDistancesToLocation(inputs, in, neighbors, current, previous, gs, targets,
				callDepth);
	}
}
