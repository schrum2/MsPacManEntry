package entrants.pacman.Vilhelm_Von_Tweann.edu.southwestern.networks;
...
import entrants.pacman.Vilhelm_Von_Tweann.edu.southwestern.evolution.genotypes.TWEANNGenotype.LinkGene;
...
import entrants.pacman.Vilhelm_Von_Tweann.edu.southwestern.util.CombinatoricUtilities;
import entrants.pacman.Vilhelm_Von_Tweann.edu.southwestern.util.graphics.DrawingPanel;
import entrants.pacman.Vilhelm_Von_Tweann.edu.southwestern.util.graphics.GraphicsUtil;
import entrants.pacman.Vilhelm_Von_Tweann.edu.southwestern.util.graphics.Plot;
...
//			}
	...
	private static void refreshActivation(DrawingPanel inputPanel, double[] inputs, double[] outputs, double[] preferences, boolean multitask, double[] preferenceFatigue) {
		...
		g.fillRect(0, (int) (0 * Plot.OFFSET) - (Plot.OFFSET / 2), inputPanel.getFrame().getWidth(), ((inputs.length + outputs.length + preferences.length + 2 + 3) / 2) * Plot.OFFSET);
		...
		g.setColor(Color.ORANGE);
		g.drawString("MODE PREFERENCES", Plot.OFFSET, (int) ((0 + (i * 0.5)) * Plot.OFFSET));
		i++;
