...
import entrants.pacman.Vilhelm_Von_Tweann.edu.southwestern.tasks.mspacman.sensors.blocks.GhostsWithinDistanceBlock;
import entrants.pacman.Vilhelm_Von_Tweann.edu.southwestern.tasks.mspacman.sensors.blocks.distancedifference.EscapeNodeDistanceDifferenceBlock;
import entrants.pacman.Vilhelm_Von_Tweann.edu.southwestern.tasks.mspacman.sensors.blocks.nearestfarthest.NearestFarthestEdibleGhostBlock;
import entrants.pacman.Vilhelm_Von_Tweann.edu.southwestern.tasks.mspacman.sensors.blocks.nearestfarthest.NearestPillBlock;
import entrants.pacman.Vilhelm_Von_Tweann.edu.southwestern.tasks.mspacman.sensors.blocks.nearestfarthest.NearestPowerPillBlock;
import entrants.pacman.Vilhelm_Von_Tweann.edu.southwestern.tasks.mspacman.sensors.blocks.proximity.ThreatGhostDirectionalProximityBlock;
...
public class StaticPacManController extends NNDirectionalPacManController {
	...
	public StaticPacManController(Network n, int depth) {
		...
		((BlockLoadedInputOutputMediator) inputMediator).blocks.add(new EscapeNodeDistanceDifferenceBlock(
				inputMediator.escapeNodes, false, false, futureSteps, !ignorePillScore, !playWithoutPowerPills, false));
		...
		((BlockLoadedInputOutputMediator) inputMediator).blocks
				.add(new GhostsWithinDistanceBlock(new boolean[] { false, false, false, true }, true, 80));
	...
	public int getDirection(GameFacade gs) {
		if (gs.ghostReversal() || gs.justAtePowerPill() || eatenGhosts < gs.getNumEatenGhosts()
	...
	public double[] getDirectionPreferences(GameFacade gf) {
		...
		int timeLevelWidth = GameFacade.NUM_DIRS + (!ignorePillScore ? GameFacade.NUM_DIRS : 0)
				+ (!playWithoutPowerPills ? GameFacade.NUM_DIRS : 0);
		...
		for (int i = 0; i < GameFacade.NUM_DIRS; i++) {
			...
			if (neighbors[dir] == -1) {
				...
				// }
				// if (inputs[i + nearestPill] == -1 && inputs[i + (2 *
				// GameFacade.NUM_DIRS) + nearestPill] == -1) { // Wall
				// outputs[i] = -100;
				causes[i] = "Wall";
			} else if (!ignorePillScore && inputs[i] > 0 && inputs[i + 8] == 1.0
					&& gf.getActivePowerPillsIndices().length == 0 && !gf.anyIsEdible()) { // can
																							// get
																							// the
																							// last
																							// pill!
				outputs[i] = 100000;
				causes[i] = "Last Pill!";
			} else if (inputs[i + deepestGroup] > 0) { // Option 4 steps ahead
														// that gets most pills
				if (gf.getActivePowerPillsIndices().length == 0 && !gf.anyIsEdible()) {
					double preference = 1;
					for (int j = 0; j < futureSteps; j++) {
						preference += 100 * Math.pow(10, j)
								* inputs[i + deepestGroup + GameFacade.NUM_DIRS - (j * timeLevelWidth)]; // Pill
																											// value
					}
					...
					for (int j = 0; j < futureSteps; j++) {
						...
						double amount = 100 * Math.pow(10, j) * portion; // Ghost
																			// value
						rewards += amount + ",";
						preference += amount;
					}
					boolean eatAll = portionEaten > 0.99 && portionEaten < 1.01; // allow
																					// wiggle
																					// room
																					// for
																					// floating
					...
					if (!gf.anyIsEdible() && preference > 1 && !eatAll) { // No
																			// edible
																			// ghosts,
																			// but
																			// there
						...
						preference = inputs[i + nearestPill] == 1 ? 0 : 0.1; // Nearest
																				// Edible
																				// Ghost
					} else if (inputs[i + edibleGhost] > 0) {
						causes[i] += "(Edible Bias " + rewards + ")";
						preference += inputs[i + edibleGhost] * 1000; // Nearest
																		// Edible
																		// Ghost
						// } else if (inputs[i + edibleGhost] > 0 && inputs[i +
						// inputs.length - GameFacade.NUM_DIRS] == -1) {
						// causes[i] += "(Safe Edible Bias " + rewards + ")";
						// preference += inputs[i + edibleGhost] * 10; //
						// Nearest Edible Ghost
					} else if (!gf.anyActiveGhostInLair() && !gf.anyIsEdible()
							&& (inputs[allNear] == 1 || RandomNumbers.randomGenerator.nextDouble() < 0.2 || eatAll)) {
						causes[i] += "(Power Pill Bias " + rewards + ")";
						preference += inputs[i + nearestPill] * 10; // Nearest
																	// Power
																	// Pill
					} else {
				...
				for (int group = deepestGroup - timeLevelWidth, depth = futureSteps
						- 1; group > GameFacade.NUM_DIRS; group -= timeLevelWidth) {
					if (inputs[i + group] > 0) {
