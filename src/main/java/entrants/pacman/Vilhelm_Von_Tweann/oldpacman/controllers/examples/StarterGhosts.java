...
public final class StarterGhosts extends NewGhostController {
	...
	private final static float CONSISTENCY = 0.9f; // attack Ms Pac-Man with
													// this probability
	private final static int PILL_PROXIMITY = 15; // if Ms Pac-Man is this close
													// to a power pill, back
													// away
		...
		for (GHOST ghost : GHOST.values()) // for each ghost
		{
													...
													// action
			{
																				...
																				// pill
				{
					...
					if (game.rnd.nextFloat() < CONSISTENCY) // attack Ms Pac-Man
															// otherwise (with
															// certain
															// probability)
					{
						myMoves.put(ghost,
								game.getApproximateNextMoveTowardsTarget(game.getGhostCurrentNodeIndex(ghost),
										game.getPacmanCurrentNodeIndex(), game.getGhostLastMoveMade(ghost), DM.PATH));
					} else // else take a random legal action (to be less
							// predictable)
					{
