...
import static entrants.pacman.Vilhelm_Von_Tweann.oldpacman.game.Constants.*;
...
/**
 * This class is the view that displays the game. The only thing contestants
 * might need to know about are the helper methods that allow one to add colours
 * and lines to the game view. This could be useful for debugging.
 */
...
public final class GameView extends JComponent {
	...
	public synchronized static void addPoints(Game game, Color color, int... nodeIndices) {
		if (isVisible) {
			for (int i = 0; i < nodeIndices.length; i++) {
				debugPointers.add(new DebugPointer(game.getNodeXCoord(nodeIndices[i]),
						game.getNodeYCoord(nodeIndices[i]), color));
			}
		}
	}
	 ...
	 * (fromNnodeIndices.length must be equals toNodeIndices.length)
	 *
	 * @param game
	 *            Copy of the current game
	 * @param color
	...
	public synchronized static void addLines(Game game, Color color, int[] fromNnodeIndices, int[] toNodeIndices) {
		if (isVisible) {
			for (int i = 0; i < fromNnodeIndices.length; i++) {
				debugLines.add(
						new DebugLine(game.getNodeXCoord(fromNnodeIndices[i]), game.getNodeYCoord(fromNnodeIndices[i]),
								game.getNodeXCoord(toNodeIndices[i]), game.getNodeYCoord(toNodeIndices[i]), color));
			}
		}
	}
	...
	public synchronized static void addLines(Game game, Color color, int fromNnodeIndex, int toNodeIndex) {
		if (isVisible) {
			debugLines.add(new DebugLine(game.getNodeXCoord(fromNnodeIndex), game.getNodeYCoord(fromNnodeIndex),
					game.getNodeXCoord(toNodeIndex), game.getNodeYCoord(toNodeIndex), color));
		}
	}

	/**
	 * Draw the debug info and the clear it - it is shown for a single time step
	 * only.
	 */
	...
	public void paintComponent(Graphics g) {
		...
		drawMaze();
		drawDebugInfo(); // this will be used during testing only and will be
							// disabled in the competition itself
		drawPills();
		drawPowerPills();
		drawPacMan();
		drawGhosts();
	...
	private void drawPills() {
		...
		for (int i = 0; i < pillIndices.length; i++) {
			if (game.isPillStillAvailable(i)) {
				bufferGraphics.fillOval(game.getNodeXCoord(pillIndices[i]) * MAG + 4,
						game.getNodeYCoord(pillIndices[i]) * MAG + 8, 3, 3);
			}
		}
	}
	...
	private void drawPowerPills() {
		...
		for (int i = 0; i < powerPillIndices.length; i++) {
			if (game.isPowerPillStillAvailable(i)) {
				bufferGraphics.fillOval(game.getNodeXCoord(powerPillIndices[i]) * MAG + 1,
						game.getNodeYCoord(powerPillIndices[i]) * MAG + 5, 8, 8);
			}
		}
	}
	...
	private void drawPacMan() {
		...
		bufferGraphics.drawImage(images.getPacMan(lastPacManMove, time), game.getNodeXCoord(pacLoc) * MAG - 1,
				game.getNodeYCoord(pacLoc) * MAG + 3, null);
	}
	...
	private void drawGhosts() {
		for (GHOST ghostType : GHOST.values()) {
			...
			int nodeXCood = game.getNodeXCoord(currentNodeIndex);
			int nodeYCood = game.getNodeYCoord(currentNodeIndex);
	...
	private void drawLives() {
		for (int i = 0; i < game.getPacmanNumberOfLivesRemaining() - 1; i++) // -1
																				// as
																				// lives
																				// remaining
																				// includes
